...
int write_results_to_file(struct room* room);
...
int main() {
    ...
    do {
        ...
        case 1:
            ...
            if (read_materials_file(fname, &materialCount) == NULL){
            ...
            scanf("%d", &amount_marerials);
            break;

        case 2:
            // Задание параметров комнаты
            set_room_parameters(&room_data);
            break;

        case 3:
            // Задание параметров проемов комнаты
            set_openings_parameters(&room_data);
            break;
...
int set_room_parameters(struct room* room) {
    ...
    room->S = (room->length1 * room->height + room->length2 * room->height) * 2; // Расчет площади стен комнаты без проемов
...
int set_openings_parameters(struct room* room) {
    ...
    room->area_of_openings = (room->window_count * room->window_width * room->window_length) + (room->door_width * room->door_length * room->door_count);

    return 0;
}

// Функция расчёта площади стен с учетом проемов
int output_of_the_result_with_openings(struct room* room) {
...
int number_of_rolls(struct room* room) {
    return (((room->S - room->area_of_openings) / 220) + 1);
}

// Функция расчёта количества клея
int output_of_glue_amount(struct room* room) {
    return ((room->S - room->area_of_openings) / 2.);
}
